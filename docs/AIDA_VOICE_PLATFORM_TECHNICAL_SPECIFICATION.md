...
Telephony: HostedPulse / Asterisk 20.9.2  
Voice agent: LiveKit  
Handset: Grandstream GXV3450 / Android 11  
Existing integrations: LocalSplash CRM, EchoWeb, EchoDatabase, EchoService  
...
HostedPulse/Asterisk owns the PSTN call from arrival through hangup. Aida joins through LiveKit as a temporary media participant. The Android Aida Handset displays call state, live transcription, suggested actions, and a **Take over** control. Take over rings the configured SIP destination and connects the human without cutting off Aida's current transfer statement.

HostedPulse owns call recording and plays the required English recording disclosure before Aida begins. The POC is English-only; language selection is deferred.
...
- **HostedPulse** is the existing Asterisk SIP/PSTN platform. It remains authoritative for channels, bridges, endpoints, ringing, answer, and hangup.
- **HostedPulse call recording** remains authoritative for recording media, disclosure playback, file lifecycle, and any existing recording retention controls. Aida does not make a second recording.
...
- **EchoDatabase** contains existing organization, user, identity, membership, session, UISP SSO, SMS-number, and carrier data.
- **EchoWeb** supplies existing Google, Microsoft, and UISP sign-in and administration patterns.
...
| **HostedPulse API** | Private Asterisk adapter/API |
...
| `api.aida.relevant.com` | Browser/handset REST API and signed LiveKit webhook receiver |

Ingress routes by hostname to internal services. The POC and foreseeable deployment use LiveKit Cloud for media and realtime transcript/control data. Pusher provides call-arrival notifications. Aida does not operate a first-party WebSocket service and no `realtime.aida.relevant.com` DNS record is required.
...
### 4.2 Private HostedPulse API

Use **`hostedpulse-api.relevant.com`**. DNS names remain lowercase even though the product label is “HostedPulse API.”

This hostname resolves to a different private or tightly restricted IP from the public wildcard. Preferred connectivity is private networking, VPN, or an overlay. If Internet-routable, firewall rules allowlist HostedPulse egress addresses and both sides require mTLS. It exposes no browser UI, permissive CORS, user login, or public API explorer.
...
Asterisk ARI remains bound to loopback or a private HostedPulse interface. Only HostedPulse API possesses ARI credentials.
...
- HostedPulse and agent event ingestion;
...
- Echo authentication/token exchange;
...
LiveKit Cloud and the already functioning HostedPulse-to-LiveKit SIP trunk are the POC media path. Asterisk passes custom `X-Aida-*` SIP headers. The POC headers are `X-Aida-Call-Session` and `X-Aida-Route-Token`; the route token is opaque, short-lived, single-call, and validated by AidaControl.
...
### 5.3 `HostedPulseAidaIntegration`

Companion service and managed configuration layered onto the existing HostedPulse server. It does not fork, rebuild, or modify Asterisk source code. Its TypeScript/Node.js service is deployed adjacent to Asterisk and is externally addressed as HostedPulse API at `hostedpulse-api.relevant.com`.
...
The POC is a separate application linked from EchoWeb and sharing Echo authentication through token exchange. EchoWeb may later embed the same Aida screens or navigation. A centralized OAuth/OIDC project is a planned successor, not a prerequisite for the POC.
...
- complete environment-variable and secret inventory, including Postgres, LiveKit API key/secret, predefined LiveKit agent ID, Pusher, NocoDB, EchoWeb token exchange, Firebase if used, CRM when enabled, and HostedPulse service credentials;
- public wildcard and private HostedPulse DNS, TLS, firewall, mTLS, and ingress instructions;
...
- EchoWeb configuration and migration instructions;
- HostedPulse integration installation order and verification;
...
- `tenant_external_reference`: tenant, system (`echo`, `localsplash_crm`, `uisp`), external ID, metadata; unique by system/external ID.
...
Echo `iOrgId` is an external reference, not Aida's primary key.
...
- `inbound_route`: tenant, DID, profile, typed destination, ring timeout, no-answer/failure policy, HostedPulse node, enabled, revision.
...
1. HostedPulse receives the DID, answers according to existing inbound policy, and starts its normal call recording.
2. HostedPulse always plays the locally stored disclosure: “Calls are recorded.”
3. HostedPulse enters the Aida Stasis context.
4. The HostedPulseAidaIntegration service creates `callSessionId` and bootstraps AidaControl with DID, caller ID, linked ID, node ID, `en-US` locale, and authenticated request data.
...
6. HostedPulse adds the custom Aida SIP correlation headers and retains the caller while bridging it to the Aida media leg through the existing LiveKit trunk.
...
3. In parallel, the HostedPulseAidaIntegration service originates the destination and reports ringing.
4. Before answer, the caller remains connected to Aida through completion of the transfer statement. If ringing continues after the statement, HostedPulse mutes/parks the Aida media leg and supplies the configured hold treatment until answer or timeout.
5. On answer, the HostedPulseAidaIntegration service immediately joins the human and blocks Aida from receiving another caller turn.
...
## 10. HostedPulse/Asterisk integration
...
exten => _X!,1,NoOp(Aida inbound ${CALLERID(num)} -> ${EXTEN})
 ...
 same => n,Stasis(aida-hostedpulse,${EXTEN},${CALLERID(num)})
...
The HostedPulseAidaIntegration service must adapt the final variables/fallback flow to installed modules and existing HostedPulse dialplan. No public HTTP call occurs synchronously from dialplan.

### 10.2 ARI/bridge rules

- Dedicated least-privilege ARI account and Stasis application.
- Only the HostedPulseAidaIntegration service holds ARI credentials.
...
- The HostedPulseAidaIntegration service reconciles active resources after restart.
...
HostedPulse stores pre-rendered English audio assets locally so entry and fallback behavior work without LiveKit or AidaControl:
...
These are project-owned recordings, not assumed Asterisk built-ins. `HostedPulseAidaIntegration` includes source WAV files and a deployment step that produces the codec/sample-rate variants required by the installed Asterisk channels (at minimum the site's negotiated narrowband format). HostedPulse always plays the normal recording disclosure first. If Aida is unavailable, it then plays the caller “circuits are busy” prompt, originates the configured extension, plays the extension-facing incident prompt after answer, and bridges the parties. Prompt version/checksum and deployment validation are documented in `AidaInfrastructureSetupInstructions`.
...
### 11.2 Private API at `hostedpulse-api.relevant.com`
...
Route validation confirms normalized/unique DID, published profile, HostedPulse node reachability, and destination existence. Publishing creates an immutable version; active calls retain their pinned version.
...
- EchoDatabase organization, user, identity, membership, session, and UISP nonce tables;
- EchoWeb Google/Microsoft OAuth, UISP HMAC SSO/replay protection, organization provisioning, sessions, business-number selection, and short-lived staff context;
- EchoWeb sends messaging operations through EchoService while currently accessing authentication data directly.

Initial integration:

- create an Aida `tenantId` and link Echo `auth_tbl_Org.iOrgId` as an external reference;
- modify EchoWeb to issue a signed, short-lived Aida token through a dedicated token-exchange endpoint;
- include user, Aida tenant, membership, permissions, session ID, issuer/audience, issue/expiry, and token ID;
- validate tokens and perform Aida authorization in AidaControl;
- define permissions such as `voice.routes.manage`, `voice.profiles.manage`, `voice.calls.control`, and `voice.calls.read`.

Echo's SMS business-phone table is not Aida voice routing. It may seed suggested DIDs with source references, but Aida supports multiple DIDs and owns voice configuration. Echo's selected `iBusinessNumber` is UI context, not sufficient authorization.

AidaAdmin is a separate public application for the POC and redirects to EchoWeb for sign-in/token exchange. EchoWeb may also link to or later embed Aida administration. A later centralized OAuth/OIDC project may own identity for both products. Until then, retain one authoritative Echo identity store and no second password database.
...
- The POC Aida-unavailable behavior is deterministic: HostedPulse first plays the normal recording disclosure, then a caller-facing “circuits are busy” prompt, rings the configured extension, and after answer plays a brief extension-facing incident prompt before joining caller and extension. If the extension does not answer, normal HostedPulse no-answer handling applies.
...
- HostedPulse records every inbound POC call and plays the recording disclosure before Aida. Recording retention, access, deletion, export, and jurisdiction-specific consent remain HostedPulse/tenant policies subject to legal review.
...
3. HostedPulse starts call recording, always plays the English disclosure, and passes `en-US` into the Call Session.
...
10. Caller-human audio and HostedPulse recording survive Aida/LiveKit removal.
...
2. `HostedPulseAidaIntegration`, including its ARI simulator and Asterisk deployment assets
3. `AidaAgent` with fake AI providers
4. `AidaHandset`
5. `AidaAdmin`
6. `AidaInfrastructureSetupInstructions`
7. Staging HostedPulse end-to-end integration using the setup repository's smoke test

AidaControl contracts and deterministic fakes are delivered first so the other projects can be built independently. HostedPulse is customized by installing the integration repository's service, include files, prompts, and configuration; Asterisk itself is never forked.

The six names above are the complete list of new GitHub repositories for the initial implementation. `EchoWeb` is an existing repository that receives the Aida token-exchange modification; it is not duplicated or counted as a new Aida repository. EchoDatabase and EchoService remain existing integrations and require changes only if their own repository-specific implementation plan explicitly identifies one.
